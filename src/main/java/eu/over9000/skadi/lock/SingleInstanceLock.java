...
public class SingleInstanceLock {
	...
	public static boolean startSocketLock() {
		try {
			SingleInstanceLock.lockingSocket = new DatagramSocket(SingleInstanceLock.SKADI_LOCKING_PORT, InetAddress.getLoopbackAddress());
					...
					while (SingleInstanceLock.lockingSocket != null && !SingleInstanceLock.lockingSocket.isClosed()) {
						...
						try {
							...
							if (Arrays.equals(SingleInstanceLock.WAKEUP_SIGNATURE, incoming.getData())) {
								System.out.println("received wakeup on locking socket");
			...
			try {
				...
				final DatagramPacket sendWakeupPacket = new DatagramPacket(SingleInstanceLock.WAKEUP_SIGNATURE, SingleInstanceLock.WAKEUP_SIGNATURE.length, InetAddress.getLoopbackAddress(),
