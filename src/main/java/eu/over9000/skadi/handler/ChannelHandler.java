...
public class ChannelHandler {
	...
	private final ObservableList<Channel> channels = FXCollections.observableArrayList(c -> new Observable[] { c.titleProperty(), c.nameProperty(), c.uptimeProperty(), c.onlineProperty(),
			c.viewerProperty(), c.gameProperty() });
	...
	public ChannelHandler(final PersistenceHandler persistenceHandler, final StateContainer state) {
		final List<Channel> emptyChannels = state.getChannels().stream().map(c -> new Channel(c, "-", "-", 0, 0)).collect(Collectors.toList());
	...
	public List<String> getChannelNames() {
		final List<String> result = this.channels.stream().flatMap(c -> Stream.of(c.getName())).sorted().collect(Collectors.toList());
		return result;
	}
	...
	public void addChannels(final Set<String> result, final StatusBar sb) {
		...
		System.out.println(this.getChannelNames());
